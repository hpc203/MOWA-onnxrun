...
public:
	MOWA();
	Mat detect(const Mat srcimg, const bool resize_flow);
private:
...
MOWA::MOWA()
{
    ...
    const int len = 1*1*this->encoder_input_height*this->encoder_input_width;
	this->mask_tensor.clear();
    this->mask_tensor.resize(len, 1.0);
}

Mat MOWA::detect(const Mat srcimg, const bool resize_flow)
{
	Mat input;
	resize(srcimg, input, Size(this->encoder_input_width, this->encoder_input_height), INTER_LINEAR);
    ...
    memcpy(this->input_tensor.data() + image_area * 2, (float *)bgrChannels[2].data, single_chn_size);
	input.release();

	std::vector<Ort::Value> encoder_inputTensors;
	encoder_inputTensors.push_back((Ort::Value::CreateTensor<float>(memory_info_handler, input_tensor.data(), input_tensor.size(), this->encoder_input_tensor_shape.data(), this->encoder_input_tensor_shape.size())));
	...
	const int img_h = this->encoder_input_height;
	const int img_w = this->encoder_input_width;
	...
	for(int n=0;n<this->head_num;n++)
	{
		...
		resample_image_xy(output_tps.data(), this->input_tensor.data(), {1, 3, img_h, img_w}, tps2flow.data(), {1, 2, img_h, img_w}); /// vector.data()返回指向数组的指针
	...
	Mat predflow_x, predflow_y;
	split2xy(final_flow, flow_shape, predflow_x, predflow_y);
	const float scale_x = (float)srcimg.cols / predflow_x.cols;
	const float scale_y = (float)srcimg.rows / predflow_x.rows;
	resize(predflow_x, predflow_x, Size(srcimg.cols, srcimg.rows));
	predflow_x *= scale_x;
	resize(predflow_y, predflow_y, Size(srcimg.cols, srcimg.rows));
	predflow_y *= scale_y;
	Mat pred_out = flow_mesh(predflow_x, predflow_y, srcimg, srcimg.rows, srcimg.cols);
...
int main()
{
	...
	string imgpath = "/home/wangbo/mowa/Datasets/Portrait Photo/0001_n21.jpg";
	...
	Mat pred_out = mynet.detect(srcimg, false);
	
	imwrite("source.jpg", srcimg);
	imwrite("Portrait Photo.jpg", pred_out);
